...
class TestListPortfolios(unittest.TestCase):
    """Tests for GET /portfolio/ using a temporary directory."""
    ...
    def test_returns_sorted_ids(self):
        """Returns portfolio IDs sorted alphabetically with suffix stripped."""
        with tempfile.TemporaryDirectory() as tmp_root:
            cv_dir = self._make_cv_dir(tmp_root)
            (cv_dir / "Zara_e5f6a7b8_Portfolio_CV.yaml").touch()
            (cv_dir / "Alice_a1b2c3d4_Portfolio_CV.yaml").touch()
            (cv_dir / "Alice_a1b2c3d4_Resume_CV.yaml").touch()  # should be ignored

            with patch("src.API.Portfolio_Generator_API.Path") as mock_path:
    ...
    def test_empty_directory_returns_empty_list(self):
        """Returns empty list when no portfolio files exist."""
        with tempfile.TemporaryDirectory() as tmp_root:
